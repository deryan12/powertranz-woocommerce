...
{
    if (is_checkout()) {
        // Google Fonts necesarias para el formulario de tarjeta
        ...
        // IMask.js desde CDN (requerido por el formulario de tarjeta de crédito)
        wp_enqueue_script('imask-js', 'https://cdnjs.cloudflare.com/ajax/libs/imask/3.4.0/imask.min.js', array(), '3.4.0', true);

        // JS del plugin (depende de IMask, jQuery opcional para eventos de checkout WC)
        ...
        // Pasar URLs al JS (mantenido por compatibilidad)
...
/**
 * Manejador de Callback AJAX (Alternativa para problemas 403/Rest)
...
{
    // Asegurar que WC esté cargado
    if (!class_exists('WC_Payment_Gateway')) {
        die('WooCommerce no está activo');
    }

    // Cargar la clase de la pasarela si es necesario
    ...
    // Los manejadores AJAX deben terminar con die
    die();
}

/**
 * Endpoint AJAX personalizado para mostrar 3DS (Omite redirecciones de WooCommerce)
 */
